...
void MyAlarm (){
    int i, status;
    for(i=0;i<pnum;i++){
        int res=waitpid(children[i], &status, WNOHANG);
        if(res==0){
...
int main(int argc, char **argv) {
  ...
  int i;
  for (i = 0; i < pnum; i++) {  // child_pid - идентификатор процесса
